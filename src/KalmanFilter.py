...
#         self.m_outputs = H.shape[0]   # no. of row vectors in H (lenght of the meas. vector)
        ...
        self.x_p = np.zeros((self.n_states, 1)) if x0 is None else x0 # prior 
        self.x_m = np.zeros((self.n_states, 1)) if x0 is None else x0 # posterior 
    ...
    # Evaluate KF based on the measurement (z) and intput (u) data
